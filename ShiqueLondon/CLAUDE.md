...
3. **Verify Google account/property context before every GA4/GTM/GSC/GBP action.** This project already hit one near-miss where the browser session silently switched to the user's own NextGen Digital account — a separate venture/Google property from Shique London, not a third-party agency, but still the wrong one to be editing in. Confirm the account/property reads "Shique London" / `shiquelondon.co.uk` before saving any change in Analytics, Tag Manager, Search Console, or Business Profile.
...
- **CMS:** WordPress 7.0.2, custom theme **Stylena** (built by A2Z Systems, the user's own venture — now archived/rebranded as **NextGen Digital**, which runs this workspace; see the ownership note below)
...
**Ownership note (added 2026-08-14):** A2Z Systems and NextGen Digital are both the user's own ventures — A2Z Systems is now archived (though some other projects are still linked to it), and NextGen Digital is its revamp. They are **not** separate/unrelated agencies. Earlier notes in this file describing NextGen Digital as a third-party "build agency" or an "unrelated" account should be read in that light — the caution about verifying the correct Google account/property before making changes (Guardrail #3) still applies, since NextGen Digital's Google properties are still distinct from Shique London's, but there's no cross-agency confusion risk, just two properties under the same user.
...
- **GTM access note**: the correct Google identity for Shique London's GA4/GTM/Search Console is a separate Google account from the agency's own `waqas.haneef1@gmail.com` (which has access to the user's other venture's NextGen Digital Google account — not an unrelated third party, just a different property) — the browser session occasionally resets to the wrong account; always verify the account/property before making changes (Tag Manager account should read "Shique London" / `shiquelondon.co.uk`, GA4 breadcrumb should read "Shique London"). **See Operating Guardrails #3.**
...
- **2026-08-14 (Cowork session)**: Project folder moved from `D:\P\A2ZSystems\Stylena\ClaudeArtifacts` to `D:\P\NextGenDigital\Claude-Artifacts\ShiqueLondon` — same files, just relocated; re-granted folder access under the new path, confirmed all four files (`CLAUDE.md`, `seo-audit-findings.md`, `strategic-plan-reference.md`, `Shique_London_SEO_Implementation_Plan.docx`) carried over intact. User clarified the org relationship prompted by the move: **A2Z Systems and NextGen Digital are both the user's own ventures**, not separate/unrelated agencies — A2Z Systems is now archived (some other projects still linked to it) and NextGen Digital is its revamp. Corrected this file's earlier framing in the Access/Site Stack/Resolved-bug/Analytics sections, which had described NextGen Digital as a third-party "build agency" or an "unrelated account" — added an ownership note under Site Stack and softened the account-mixup language accordingly. The underlying operational caution (Guardrail #3: verify the correct Google account/property before every change) still stands, since NextGen Digital's Google properties remain distinct from Shique London's even though both belong to the same user.
